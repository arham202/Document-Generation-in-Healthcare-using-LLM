...
class SugarValidation extends StatefulWidget {
  @override
  _SugarValidationState createState() => _SugarValidationState();
}

class _SugarValidationState extends State<SugarValidation> {
  ...
  List<String> _mealTimingOptions = ['After', 'Before', 'Other'];
  ...
  Future<void> _fetchDataFromServer() async {
    try {
      final response = await http.post(
        Uri.parse('http://10.0.2.2:5000/api/get_data_sugar'),
      ...
      if (response.statusCode == 200) {
        ...
        if (receivedQ1 != null && receivedQ2 != null) {
          setState(() {
            ...
            if (receivedQ2 == 'after') {
              _mealTimingOptions = ['After', 'Before', 'Other'];
            } else if (receivedQ2 == 'before') {
              _mealTimingOptions = ['Before', 'After', 'Other'];
            } else if (receivedQ2 == 'other') {
              _mealTimingOptions = ['Other', 'Before', 'After'];
            }

            // If _receivedQ2 is not present in _mealTimingOptions, set it to a default value
            if (!_mealTimingOptions.contains(_receivedQ2)) {
              _receivedQ2 = _mealTimingOptions.first;
            }
          });
        } else {
  ...
  Future<void> _submitData() async {
    try {
      String mealTiming = _selectedDropdownValue.isNotEmpty
          ? _selectedDropdownValue
          : _mealTimingOptions.first;
      ...
      final response = await http.post(
        Uri.parse('http://10.0.2.2:5000/api/submit_data_sugar'),
        headers: {'Content-Type': 'application/json'},
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                value: _mealTimingOptions.contains(_selectedDropdownValue)
                    ? _selectedDropdownValue
                    : _mealTimingOptions.first,
